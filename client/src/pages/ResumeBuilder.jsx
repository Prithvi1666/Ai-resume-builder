...
import { ArrowLeftIcon, Briefcase, ChevronLeft, ChevronRight, DownloadIcon, EyeIcon, EyeOffIcon, FileText, FolderIcon, GraduationCap, SaveAll, Share2Icon, Sparkles, User } from 'lucide-react'
...
const ResumeBuilder = () => {
  ...
  const currentResumeId = resumeId || resumeID
  ...
  const loadExistingResume = async () => {
    try {
      const {data} = await api.get('/api/resumes/get/' + currentResumeId, {headers: { Authorization: token}})
      if(data.resume){
  ...
  const handleShare = () =>{
    ...
    const resumeUrl = frontendUrl + '/view/' + currentResumeId;
  ...
  const saveResume = async () => {
    try {
      ...
      console.error("Error saving resume:", error)
      throw new Error(error?.response?.data?.message || "Unable to save resume")
    }
  }
  ...
  return (
    <div>
        ...
        <div className='max-w-7xl mx-auto px-4 py-6'>
          <button onClick={() => navigate('/app')} className='inline-flex gap-2 items-center text-slate-500 hover:text-slate-700 transition-all'>
          ...
          <div className='grid lg:grid-cols-12 gap-8'>
            ...
            <div className='relative lg:col-span-5 rounded-lg overflow-hidden '>
              <div className='bg-white rounded-lg shadow-sm border border-gray-200 p-6 pt-1'>
                ...
                <hr className='absolute top-0 left-0 h-1 bg-gradient-to-r from-green-500 to-green-600 border-none transition-all duration-2000 '
                  ...
                  <div className='flex justify-between items-center mb-6 border-b border-gray-300 py-1 '>
                    ...
                    <div className='flex items-center'>
                      {activeSectionIndex !== 0 && (
                        ...
                        gap-1 p-3 rounded-lg text-sm font-medium text-gray-600 hover:bg-gray-50 transition-all ' disabled={activeSectionIndex === 0}>
                          <ChevronLeft className='size-4 '/> Previous
                        </button>
                      )}
                      <button onClick={()=> setActiveSectionIndex((prevIndex)=> Math.min(prevIndex + 1, section.length -1))} className={`flex items-center 
                      gap-1 p-3 rounded-lg text-sm font-medium text-gray-600 hover:bg-gray-50 transition-all ${activeSectionIndex === section.length -1 ? 'opacity-50' : ''}`} 
                  ...
                  <button onClick={()=> {toast.promise(saveResume(), {loading: 'Saving...', success: (message) => message, error: (err) => err?.message || 'Failed to save resume'})}} className='bg-gradient-to-br from-green-100 to-green-200 ring-green-300 text-green-600 ring 
            ...
            <div className='lg:col-span-7 max-lg:mt-6'>
              <div className='relative w-full'>
                <div className='absolute bottom-3 left-0 right-3 flex items-center justify-end gap-2'>
                  {resumeData.public && (
                    <button onClick={handleShare} className='flex items-center p-2 px-4 gap-2 text-xs bg-gradient-to-br from-blue-100 to-blue-200 text-blue-600
                     rounded-lg ring-blue-3300 hover:ring transition-colors'>
                  ...
                  <button onClick={downloadeResume} className='flex items-center gap-2 px-6 py-2 text-xs bg-gradient-to-br from-green-100 to-green-200 text-green-600 
